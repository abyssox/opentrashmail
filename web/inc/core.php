...
function loadEmailJson(string $email, string $id): ?array
{
    $file = getDirForEmail($email) . DS . $id . '.json';
...
function getEmail(string $email, string $id): ?array
{
    return loadEmailJson($email, $id);
}

function getRawEmail(string $email, string $id): ?string
{
    $data = loadEmailJson($email, $id);
...
function emailIDExists(string $email, string $id): bool
{
    $file = getDirForEmail($email) . DS . $id . '.json';
...
function getEmailsOfEmail($email, bool $includeBody = false, bool $includeAttachments = false): array
{
    $result   = [];
    $settings = loadSettings();

    $isAdminInbox = !empty($settings['ADMIN']) && $settings['ADMIN'] === $email;

    if ($isAdminInbox) {
        $addresses = listEmailAddresses();
            ...
            if (
                ...
                $row['attachments'] = array_map(
                    static function ($attachment) use ($settings, $address) {
                        return $settings['URL'] . '/api/attachment/' . $address . '/' . $attachment;
                    },
                    $parsed['attachments']
                );
            }
...
function listEmailAddresses(): array
{
    $o = array();
...
function attachmentExists($email,$id,$attachment=false): bool
{
    $file = getDirForEmail($email).DS.'attachments'.DS.$id.(($attachment)?'-'.$attachment:'');
    return is_file($file);
}

function listAttachmentsOfMailID(string $email, string $id): array
{
    $file = getDirForEmail($email) . DS . $id . '.json';
...
function deleteEmail($email,$id): bool
{
    $dir = getDirForEmail($email);
...
function loadSettings(): array|false
{
    ...
    $settingsLoaded = true;
    $configFile = ROOT . DS . '..' . DS . 'config.ini';
    ...
    $settings = is_array($parsed) ? $parsed : false;
...
function array2ul($array): string
{
    $out = "<ul>";
...
function tailShell($filepath, $lines = 1): string
{
    ob_start();
...
function getUserIP(): string
{
    ...
    // Fallbacks – always treat as strings
    $client  = (string) ($_SERVER['HTTP_CLIENT_IP']       ?? '');
    $forward = (string) ($_SERVER['HTTP_X_FORWARDED_FOR'] ?? '');
    $remote  = (string) ($_SERVER['REMOTE_ADDR']          ?? '');

    // X-Forwarded-For can contain multiple IPs - use the first valid one
    if ($forward !== '') {
...
function isIPInRange( $ip, $range ): bool
{
...
function countEmailsOfAddress($email): int
{
    $count = 0;
...
function getWebhookConfig(string $email): ?array
{
...
function saveWebhookConfig($email, $config): bool
{
...
function deleteWebhookConfig($email): bool
{
    $webhookFile = getDirForEmail($email).DS.'webhook.json';
