...
function getEmailsOfEmail($email, $includeBody = false, $includeAttachments = false): array
{
    $o        = [];
    $settings = loadSettings();

    if (!empty($settings['ADMIN']) && $settings['ADMIN'] === $email) {
        $emails = listEmailAddresses();
        if (count($emails) > 0) {
            foreach ($emails as $emailAddress) {
                $dir = getDirForEmail($emailAddress);
                ...
                if ($handle = opendir($dir)) {
                    while (false !== ($entry = readdir($handle))) {
                        ...
                        $json = json_decode(file_get_contents($dir . DS . $entry), true);
                        ...
                        if ($includeAttachments && !empty($json['parsed']['attachments']) && is_array($json['parsed']['attachments'])) {
...
function removeScriptsFromHtml(string $html): string
{
    // Remove script tags
    $html = preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', '', $html) ?? $html;

    // Remove event attributes that execute scripts (onClick, onLoad, etc.)
    $html = preg_replace('/\son\w+="[^"]*"/i', '', $html) ?? $html;

    // Remove href attributes that execute scripts (javascript:)
    $html = preg_replace('/\shref="javascript[^"]*"/i', '', $html) ?? $html;

    // Remove any other attributes that execute scripts
    $html = preg_replace('/\b\w+="[^"]*\bon\w+="[^"]*"[^>]*>/i', '', $html) ?? $html;
